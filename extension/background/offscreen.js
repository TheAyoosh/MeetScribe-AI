...
chrome.runtime.onMessage.addListener(async (msg) => {
  if (msg.type === 'START_AUDIO_CAPTURE') {
    await startCapture(msg.tabId, msg.meetingId, msg.streamId);
  } else if (msg.type === 'STOP_AUDIO_CAPTURE') {
    stopCapture();
  }
});

async function startCapture(tabId, mId, streamId) {
  meetingId = mId;
  try {
    // In MV3, we use getUserMedia with a streamId provided by the service worker
    if (!streamId) throw new Error('Missing streamId for capture');
